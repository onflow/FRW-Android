...
class ExploreDAppTagItemPresenter(
    ...
    override fun bind(model: DAppTagModel) {
        with(binding) {
            ...
            textView.setTextColor(if (model.isSelected) R.color.colorSecondary.res2color() else R.color.contrast_text.res2color())

            root.strokeColor = if (model.isSelected) R.color.colorSecondary.res2color() else R.color.contrast_text.res2color()
